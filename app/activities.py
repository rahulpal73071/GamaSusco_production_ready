...
async def get_activities(
        company_id: int,
        current_user: User = Depends(get_current_user),  # ← AUTHENTICATION REQUIRED
        scope: Optional[List[int]] = Query(None, description="Filter by multiple scopes (e.g. ?scope=1&scope=2&scope=3)"),
    ...
    **Filters:**
    ...
    # Apply filters
    if scope:
        query = query.filter(EmissionActivity.scope_number.in_(scope))
    if category:
